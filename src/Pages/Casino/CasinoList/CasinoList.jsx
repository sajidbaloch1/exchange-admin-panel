...
import { changeStatus, getAllCasino } from "../casinoService";
...
export default function CasinoList() {
  ...
  const [casinoStatus, setCasinoStatus] = useState({}); // status and loading state of each casino

  const updateCasinoStatus = (id, key, value) => {
    setCasinoStatus((prev) => ({ ...prev, [id]: { ...prev[id], [key]: value } }));
  };

  const toggleHighlight = async (id, isVisible) => {
    updateCasinoStatus(id, "loading", true);
    try {
    ...
    updateCasinoStatus(id, "loading", false);
  };
      ...
      cell: (row) => (
        <div className="material-switch mt-4 d-flex align-items-center" key={row._id}>
          <input
            ...
            onChange={() => toggleHighlight(row._id, casinoStatus[row._id]?.isVisible)}
            checked={casinoStatus[row._id]?.isVisible || false}
            type="checkbox"
          />
          <label htmlFor={`highlightSwitch_${row._id}`} className="label-primary"></label>
          {casinoStatus[row._id]?.loading ? (
      ...
      cell: (row) => (
        <div>
          <OverlayTrigger placement="top" overlay={<Tooltip> Click here to edit</Tooltip>}>
            <Link
              to={`${process.env.PUBLIC_URL}/casino-form`}
              state={{ id: row._id }}
  ...
  const fetchData = async (page, sortBy, direction, searchQuery) => {
    setLoading(true);
    try {
      const result = await getAllCasino({
        page: page,
        perPage: perPage,
        sortBy: sortBy,
        direction: direction,
      ...
      const initialCasinoStatus = result.records.reduce((acc, casino) => {
        acc[casino._id] = { isVisible: casino.isVisible, loading: false };
        return acc;
      }, {});
      setCasinoStatus(initialCasinoStatus);
  ...
  const handleDownload = async () => {
    await downloadCSV("casino/getAllCasino", searchQuery, "casino.csv");
  };
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-title">ALL CASINO</h1>
        ...
        {permission.COMPETITIONS.ACTIVE && (
          <div className="ms-auto pageheader-btn">
            <Link to={`${process.env.PUBLIC_URL}/casino-form`} className="btn btn-primary btn-icon text-white me-3">
              <span>
