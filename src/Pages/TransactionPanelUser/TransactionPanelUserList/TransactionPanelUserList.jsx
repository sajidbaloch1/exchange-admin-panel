...
import { deleteTransactionUser, getAllTransactionUsers } from "../transactionPanelUserService";
import { Notify } from "../../../utils/notify";

export default function TransactionPanelUserList() {
      ...
      cell: (row) => (
        <div className="d-flex justify-content-end align-items-center">
          <OverlayTrigger placement="top" overlay={<Tooltip > Click here to edit</Tooltip>}>
            <Link
              to={`${process.env.PUBLIC_URL}/transaction-panel-user-form`}
              state={{ id: row._id }}
  ...
  const fetchData = async (page) => {
    setLoading(true);
    try {
      const result = await getAllTransactionUsers({
        page: page,
        perPage: perPage,
        sortBy: sortBy,
        direction: direction,
  ...
  const removeRow = async (id) => {
    setLoading(true);
    try {
      const success = await deleteTransactionUser(id);
      if (success) {
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-title">ALL TRANSACTION PANEL USERS</h1>
        </div>
